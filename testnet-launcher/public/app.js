...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchNodes() {
    ...
    try {
      // neues Backend: /api/nodes/active -> { active_nodes: [...] }
      const res = await fetch(`${API_BASE}/nodes/active`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      const nodes = Array.isArray(data) ? data : data.active_nodes || [];
      renderNodes(nodes);
    } catch (err) {
  ...
  async function fetchProofs() {
    const tbody = document.getElementById("proofs-body");
    try {
      // neues Backend: /api/proofs/recent -> { proofs: [...] }
      const res = await fetch(`${API_BASE}/proofs/recent`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
